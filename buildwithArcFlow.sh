if [ -v CPLEX_HOME ] ; then
	IBMPATH=${CPLEX_HOME}
else
	IBMPATH=/opt/ibm/ILOG/CPLEX_Studio1210/
fi

CFLAGS="-g -O0 -DPLOT -I${IBMPATH}cplex/include -I${IBMPATH}concert/include -I${IBMPATH}opl/include -c "

if [ $OSTYPE = "linux-gnu" ] ; then
	if [ -v GUROBI_HOME ]; then
		GUROBIPATH=${GUROBI_HOME}/linux64/include
		#path for gurobi include folder
		LIB=${GUROBI_HOME}/linux64/lib
	else
 		GUROBIPATH=/opt/gurobi912/linux64/include
        	#path for gurobi library folder
                LIB=/opt/gurobi912/linux64/lib
	fi
else
        #path for gurobi include folder
        GUROBIPATH=/Library/gurobi911/mac64/include/
        #path for gurobi library folder
        LIB=/Library/gurobi911/mac64/lib
fi
CPPLIB="-L${LIB} -lgurobi_c++ -lgurobi91"
...
g++ -shared -fPIC CFallingLoader.cpp -o CFallingLoader.so
g++ -shared -fPIC C6Loader.cpp -o C6Loader.so
g++ -shared -fPIC TestLoader.cpp -o TestLoader.so
g++ -c -fPIC -std=c++11 ArcFlow.cpp -I${GUROBIPATH} -o ArcFlow.o
if [ $OSTYPE = "linux-gnu" ] ; then
	echo "Compile for Linux"
        g++ -shared -fPIC -std=c++11 -L$LIB ArcFlow.o Machine.o $LIB/libgurobi_g++5.2.a -o ArcFlow.so -lgurobi91
else
	echo "Compile for MAC"
	g++ -shared -fPIC -std=c++11 ArcFlow.o Machine.o -o ArcFlow.so $CPPLIB
fi
g++ -c -fPIC CNS.cpp -o CNS.o
g++ -shared -fPIC CNS.o Machine.o -o CNS.so
g++ -c -fPIC SA.cpp -o SA.o
g++ -shared -fPIC SA.o Machine.o -o SA.so
...
g++ -c -fPIC -w -Wcpp -Wno-deprecated -DIL_STD $CFLAGS LTAArand.cpp -o LTAArand.o
g++ -shared -fPIC -pthread LTAArand.o Machine.o ${IBMPATH}cplex/lib/x86-64_linux/static_pic/libilocplex.a ${IBMPATH}cplex/lib/x86-64_linux/static_pic/libcplex.a ${IBMPATH}concert/lib/x86-64_linux/static_pic/libconcert.a ${IBMPATH}opl/lib/x86-64_linux/static_pic/libilocplex.a  -o LTAArand.so
g++ -c -fPIC -w -Wcpp -Wno-deprecated -DIL_STD $CFLAGS LTAAdet.cpp -o LTAAdet.o
g++ -shared -fPIC -pthread LTAAdet.o Machine.o ${IBMPATH}cplex/lib/x86-64_linux/static_pic/libilocplex.a ${IBMPATH}cplex/lib/x86-64_linux/static_pic/libcplex.a ${IBMPATH}concert/lib/x86-64_linux/static_pic/libconcert.a ${IBMPATH}opl/lib/x86-64_linux/static_pic/libilocplex.a  -o LTAAdet.so
g++ -c -fPIC -w -Wcpp -Wno-deprecated -DIL_STD $CFLAGS ChekuriVS.cpp -o ChekuriVS.o 
g++ -shared -fPIC ChekuriVS.o Machine.o -o ChekuriVS.so
